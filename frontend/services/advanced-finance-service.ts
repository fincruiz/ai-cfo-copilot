import { api } from '@/lib/api'; import type {ApiResponse} from '@/types/finance'; import type {ForecastRun,PlanningVersion,Artifact,DecisionSimulation,PlanningContext,PlanningBaseline} from '@/types/advanced-forecasting';
export const advancedFinanceService={
 ...
 async planningContext():Promise<PlanningContext>{return (await api.get<ApiResponse<PlanningContext>>('/native-planning/context')).data.data},
 async planningBaseline():Promise<PlanningBaseline>{return (await api.get<ApiResponse<PlanningBaseline>>('/advanced-forecast/baseline')).data.data},
 ...
 async reseedVersion(id:string):Promise<PlanningVersion>{return (await api.post<ApiResponse<PlanningVersion>>(`/native-planning/versions/${id}/reseed`)).data.data},
 async allocateBudget(id:string,payload:Record<string,unknown>):Promise<PlanningVersion>{return (await api.post<ApiResponse<PlanningVersion>>(`/native-planning/versions/${id}/allocate`,payload)).data.data},
