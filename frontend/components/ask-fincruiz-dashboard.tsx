...
export function AskFinCruizDashboard() {
  ...
  if (collapsed) {
    return (
      <section className="rounded-2xl border border-indigo-200/70 bg-gradient-to-r from-indigo-50 via-background to-sky-50 shadow-sm dark:border-indigo-500/20 dark:from-indigo-950/30 dark:via-background dark:to-sky-950/20">
        <button
          type="button"
          onClick={() => setCollapsedPreference(false)}
          className="flex w-full items-center justify-between gap-4 px-4 py-3 text-left sm:px-5"
          aria-expanded="false"
          aria-label="Expand Ask FinCruiz"
        >
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold">Ask FinCruiz</p>
              <p className="truncate text-xs text-muted-foreground">
                Ask a question, investigate evidence or model a decision.
              </p>
            </div>
          </div>

          <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border bg-background/80 px-3 py-1.5 text-xs font-semibold text-indigo-700 shadow-sm dark:text-indigo-300">
            Open
  ...
  return (
    <section className="overflow-hidden rounded-[28px] border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-background to-sky-50 shadow-sm dark:border-indigo-500/20 dark:from-indigo-950/30 dark:via-background dark:to-sky-950/20">
      <div className="flex items-center justify-between gap-3 border-b border-indigo-100/80 px-4 py-3 dark:border-white/10 sm:px-6">
        <div className="flex min-w-0 items-center gap-2">
          <Sparkles className="size-4 shrink-0 text-indigo-600 dark:text-indigo-300" />
          <p className="truncate text-xs font-semibold uppercase tracking-[.14em] text-indigo-700 dark:text-indigo-300">
            FinCruiz management intelligence
          </p>
        </div>
        ...
        <button
          ...
          className="inline-flex shrink-0 items-center gap-1.5 rounded-full border bg-background/80 px-3 py-1.5 text-xs font-semibold text-muted-foreground transition hover:text-foreground"
          aria-expanded="true"
      ...
      <div className="grid gap-0 lg:grid-cols-[.72fr_1.28fr]">
        <div className="border-b border-indigo-100/80 p-6 dark:border-white/10 lg:border-b-0 lg:border-r">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[.18em] text-indigo-700 dark:text-indigo-300">
                Conversational BI
              </p>
              <h2 className="text-xl font-semibold">Ask FinCruiz</h2>
            </div>
          </div>

          <p className="mt-4 text-sm leading-6 text-muted-foreground">
            Ask the management question in plain English. FinCruiz selects the
            relevant finance evidence, visualizes it where useful, and routes
            decisions into the appropriate model.
          </p>
                ...
                <div className="space-y-2">
                  {group.items.map((prompt) => (
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 rounded-2xl border border-indigo-200 bg-white/80 px-3.5 py-2.5 text-left text-xs font-medium text-slate-700 transition hover:-translate-y-0.5 hover:border-indigo-400 hover:shadow-sm dark:border-white/10 dark:bg-white/5 dark:text-slate-200"
                    >
                      <span>{prompt}</span>
        ...
        <div className="p-6">
          ...
          <label className="mt-3 flex cursor-pointer items-center gap-2 text-xs text-muted-foreground">
            ...
            Use live industry and economic context when relevant
          </label>
          ...
          <div className="mt-5 min-h-32 rounded-2xl border border-dashed bg-background/60 p-4">
            {loading ? (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <Loader2 className="size-4 animate-spin" />
                Reviewing company evidence and choosing the right analysis...
              </div>
            ) : result ? (
              <div className="animate-step-in">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.14em] text-indigo-700 dark:text-indigo-300">
                    <Sparkles className="size-3.5" />
                    FinCruiz response
                  </div>

                  <span className="rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
                ...
                result.interpreted_question !== question ? (
                  <p className="mt-3 text-[11px] text-muted-foreground">
                    Following the earlier question, FinCruiz interpreted this
                    as:{" "}
                ...
                <div className="mt-3 rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-white to-indigo-50/60 p-4 dark:border-indigo-500/20 dark:from-white/[.04] dark:to-indigo-950/20">
                  <p className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[.14em] text-indigo-700 dark:text-indigo-300">
                    <Lightbulb className="size-3.5" />
                    Management answer
                  </p>

                  <p className="mt-2 whitespace-pre-wrap text-[15px] font-medium leading-7">
                    {result.answer}
                  </p>
                </div>
                ...
                {result.evidence?.length || result.confidence ? (
                  <div className="mt-4 rounded-2xl border bg-white/70 p-4 dark:bg-white/[.03]">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {result.decision_handoff ? (
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:brightness-110"
                    >
                  ...
                  {result.external_context_used ? (
                    <span className="inline-flex items-center gap-1.5 rounded-xl border bg-background px-3 py-2 text-xs text-muted-foreground">
                      <Globe2 className="size-3.5" />
                      ...
                      {result.suggested_questions
                        ...
                        .map((suggestion) => (
                          <button
                            ...
                            className="rounded-full border bg-background px-3 py-1.5 text-xs transition hover:border-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/20"
                          >
                            {suggestion}
                          </button>
                        ))}
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-2">
                      {result.sources.slice(0, 4).map((source) => (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-start gap-2 rounded-xl border bg-background p-3 text-xs hover:border-indigo-300"
                        >
              ...
              <p className="text-sm leading-6 text-muted-foreground">
                Your answer will appear here. Financial values come from
                FinCruiz&apos;s prepared finance context; the text of your
                question is not sent to product-usage analytics.
              </p>
            )}
          </div>
        </div>
