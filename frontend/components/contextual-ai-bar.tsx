...
export function ContextualAIBar() {
  ...
  // The dashboard has the full-company Global Copilot. Avoid a duplicate page assistant there.
    ...
    <div className="sticky bottom-0 z-20 mt-8 pb-2 pt-3 pointer-events-none">
      <div className="pointer-events-auto mx-auto max-w-5xl rounded-[22px] border border-indigo-200/70 bg-background/95 p-2.5 shadow-[0_18px_55px_rgba(15,23,42,.14)] backdrop-blur-xl dark:border-indigo-500/20">
        <form onSubmit={submit} className="flex items-center gap-2">
          ...
          <div className="hidden min-w-0 sm:block"><p className="text-[10px] font-bold uppercase tracking-[.14em] text-indigo-600 dark:text-indigo-300">Ask about this page</p><p className="max-w-36 truncate text-[11px] text-muted-foreground">{context.label}</p></div>
          <input value={question} onChange={(event) => setQuestion(event.target.value)} placeholder={`Ask about ${context.label}…`} className="min-w-0 flex-1 bg-transparent px-2 text-sm outline-none placeholder:text-muted-foreground"/>
