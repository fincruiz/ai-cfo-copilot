...
export default function DemoPage(){
  ...
  const answers:Record<string,string>={
    ...
    "Can we afford to hire 3 people?":"FinCruiz routes this to the three-way forecast because hiring affects profit, cash and the balance sheet together. Model the salary cost, timing and working-capital assumptions before deciding.",
  };
  return <main className="min-h-screen overflow-hidden bg-[#07101f] text-white">
    ...
    <section className="relative z-10 mx-auto max-w-7xl px-5 pb-16 pt-8 lg:px-8">
      ...
      <div className="mt-5 rounded-[30px] border border-white/10 bg-white/[.04] p-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><div className="mb-2 inline-flex items-center gap-2 rounded-full border border-indigo-300/15 bg-indigo-300/10 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-indigo-200"><LineChart className="size-3.5"/>Three-way decision model</div><p className="font-bold">Scenario simulator</p><p className="mt-1 text-sm text-slate-400">One assumption flows through Profit & Loss, Balance Sheet and Cash Flow together.</p></div><div className="flex gap-2">{(["downside","base","upside"] as const).map(x=><button key={x} onClick={()=>setScenario(x)} className={`rounded-xl px-4 py-2 text-xs font-bold capitalize ${scenario===x?"bg-white text-slate-950":"border border-white/10 bg-white/5 text-slate-300"}`}>{x}</button>)}</div></div><div className="mt-6 grid gap-3 sm:grid-cols-4"><div className="rounded-xl bg-slate-950/45 p-4"><p className="text-xs text-slate-500">Sales growth</p><p className="mt-2 text-2xl font-bold">{scenarioData.sales}</p></div><div className="rounded-xl bg-slate-950/45 p-4"><p className="text-xs text-slate-500">Forecast cash</p><p className="mt-2 text-2xl font-bold">{scenarioData.cash}</p></div><div className="rounded-xl bg-slate-950/45 p-4"><p className="text-xs text-slate-500">Forecast profit</p><p className="mt-2 text-2xl font-bold">{scenarioData.profit}</p></div><div className="rounded-xl bg-slate-950/45 p-4"><p className="text-xs text-slate-500">Assumption</p><p className="mt-2 text-sm leading-6">{scenarioData.note}</p></div></div></div>

      <div className="mt-16 rounded-[30px] border border-white/10 bg-white/[.035] p-6"><p className="text-xs font-bold uppercase tracking-[.18em] text-indigo-300">Power underneath the simple experience</p><h2 className="mt-3 text-2xl font-bold">Nothing is hidden. FinCruiz reveals the right capability when it becomes relevant.</h2><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Financials","P&L, Balance Sheet, KPIs"],["Cash","Working capital, AR/AP"],["Plan","Forecast, scenarios, three-way model"],["Management","Intelligence, board packs, benchmarking"]].map(([title,text])=><div key={title} className="rounded-2xl border border-white/10 bg-slate-950/40 p-4"><p className="font-semibold">{title}</p><p className="mt-2 text-sm text-slate-400">{text}</p></div>)}</div></div>
