...
export default function NativePlanning() {
  ...
  async function allocate() { if (!selected) return; setLoading(true); setError(""); try { const annual_targets = Object.fromEntries(Object.entries(targets).filter(([,v]) => Number.isFinite(v))); setSelected(await advancedFinanceService.allocateBudget(selected.id,{annual_targets,detail_level:allocationDetail,seasonality})); setHistory([]); } catch(e){setError(getApiErrorMessage(e))} finally{setLoading(false)} }
    ...
    <div className="grid gap-5 xl:grid-cols-[350px_1fr]">
      ...
      <div className="space-y-5">
        {!selected?<Card><CardContent className="flex min-h-[420px] flex-col items-center justify-center gap-4 text-center"><div className="flex size-14 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-700"><Sparkles className="size-6"/></div><div><p className="font-semibold">Choose or build a budget</p><p className="mt-1 max-w-xl text-sm text-muted-foreground">FinCruiz can seed the next financial period from the last mapped actuals even though those actuals belong to a different year.</p></div></CardContent></Card>:<>
          ...
          <Card><CardHeader><CardTitle>Management targets</CardTitle><CardDescription>Enter annual high-level targets. FinCruiz can spread them over the period using historical seasonality and, if requested, allocate them to the mapped COA using historical account ratios.</CardDescription></CardHeader><CardContent className="space-y-4"><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{groups.slice(0,7).map(g=><label key={g} className="space-y-1 text-xs"><span>{g}</span><Input type="number" step="0.01" value={targets[g]??0} onChange={e=>setTargets({...targets,[g]:Number(e.target.value)})}/></label>)}</div><div className="flex flex-wrap items-end gap-3"><label className="space-y-1 text-xs"><span>Allocate to</span><select className="h-10 rounded-md border bg-background px-3" value={allocationDetail} onChange={e=>setAllocationDetail(e.target.value)}><option value="high_level">Keep high level</option><option value="detailed">Mapped GL / COA</option></select></label><label className="space-y-1 text-xs"><span>Monthly spread</span><select className="h-10 rounded-md border bg-background px-3" value={seasonality} onChange={e=>setSeasonality(e.target.value)}><option value="historical">Historical monthly pattern</option><option value="equal">Equal monthly split</option></select></label><Button onClick={()=>void allocate()} disabled={loading}><WandSparkles className="size-4"/>Apply targets</Button></div></CardContent></Card>
