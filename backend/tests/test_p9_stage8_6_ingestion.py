...
def test_safe_filename_accepts_supported_gl_files_and_rejects_unsupported():
    assert safe_filename("ledger.csv") == "ledger.csv"
    assert safe_filename("ledger.xlsx") == "ledger.xlsx"

    for filename in ("ledger.xls", "ledger.txt", "ledger.pdf", "ledger.exe"):
...
def test_stream_mapping_detects_required_columns(tmp_path: Path):
    ...
    p.write_text(
        "transaction_date,source_account_code,debit,credit\n2026-01-01,4000,0,100\n",
        encoding="utf-8",
    )
